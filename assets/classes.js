MB = {
	...
	loadServer: function(dlg){
		...
		$.get('server.php?do=getLists', function(r){
	...
	getServerList: function(id) {
		$.post('server.php?do=loadList',{id:id},function(r){
			if (r == 'error') return;
			var lists = JSON.parse(r);
			MB.removeAll();
			for(var i in lists) {
	...
	saveServer: function(){
		...
		$.post('server.php?do=saveList', lists, function(r){
			if (r == 'OK')
...
DL = {
	...
	getFiles: function(t) {
		...
		cur = t;
		VK.Api.call('audio.search', {q: q, sort: 0, count: 200, offset: 0, v: 3, test_mode: 1}, DL.onGetFiles);
	},
