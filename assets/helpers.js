...
Storage.prototype.getObject = function(key) {
    return localStorage.getItem(key) && JSON.parse(this.getItem(key));
}
...
function checkTitle(orig, title) {
	var words = orig.split(' '), wrong = 0;
	for(var k in words) {
		if (!words.hasOwnProperty(k)) continue;
		if (words[k].length < 2) continue;
		if (lc(title).indexOf(lc(words[k])) == -1) wrong++;
	}
	if (wrong/words.length > 0.5) return false;
	return true;
}
